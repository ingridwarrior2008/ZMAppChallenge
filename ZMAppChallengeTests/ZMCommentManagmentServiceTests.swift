...
class ZMCommentManagmentServiceTests: XCTestCase {
    
    var networkManager: ZMDataManagerMockNetworkProvider?
    
    func testCommentsWithSomeValuesNotPresent() throws {
        networkManager = ZMDataManagerMockNetworkProvider("mockCommentsNotValues")
        guard let networkManager = networkManager else {
            XCTAssertThrowsError("networkManager it's nil")
            return
        }
        
        let commentsServices = ZMCommentManagmentService(networkManager: networkManager)
    ...
    func testParsingCommentsResponse() throws {
        networkManager = ZMDataManagerMockNetworkProvider("mockComments")
        guard let networkManager = networkManager else {
            XCTAssertThrowsError("networkManager it's nil")
            return
        }
